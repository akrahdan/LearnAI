...
import requests
import json
from .rave_exceptions import ServerError, RefundError
import base64
...
class RaveBase(object):
    ...
    def __init__(self, publicKey=None, secretKey=None, production=False, usingEnv=True):
        ...
        self._baseUrlMap = [
            "https://ravesandboxapi.flutterwave.com/", "https://api.ravepay.co/"]
        ...
        # If we are not using environment variables
            ...
            if (not publicKey) or (not secretKey):
                ...
                warnings.warn(
                    "Though you can use the usingEnv flag to pass secretKey as an argument, it is advised to store it in an environment variable, especially in production.", SyntaxWarning)
        ...
        if production:
            ...
            self._baseUrl = self._baseUrlMap[1]
        else:

            self._baseUrl = self._baseUrlMap[0]
